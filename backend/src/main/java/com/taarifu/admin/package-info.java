 ...
 *   <li><b>User &amp; role management</b> ({@code /admin/users}) — list accounts (masked PII), view one
 *       account's detail (roles + scopes), and grant/revoke roles + suspend/reinstate, delegated to the
 *       {@code identity} module through its published {@link com.taarifu.identity.api.UserAdminQueryApi}
 *       (reads) and {@link com.taarifu.identity.api.UserAdminApi} (commands) ports; conflict-of-interest
 *       fenced (D16 — no self-action).</li>
 ...
 * </ul>
 *
 * <p>Isolation: cross-module reads/commands go only through the callees' published {@code api} ports
 * (ADR-0013 §1), injected as interfaces and implemented by a {@code @Service} in the owning module; other
 * modules are referenced by opaque {@code UUID}. The admin module imports no other module's
 * {@code domain}/{@code infrastructure} (ARCHITECTURE §3.2) — the {@code ModuleBoundaryTest} enforces it.</p>
 */
