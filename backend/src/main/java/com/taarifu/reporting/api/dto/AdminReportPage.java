 ...
 * (ADR-0013 §1 — only DTOs/enums/UUIDs cross). Shares the same {@code (content, page, size, totalElements)}
 * shape as the other published api page records (e.g. {@code identity.api.dto.UserAdminPage}) so the admin
 * module maps any port page to a {@code PageMeta} identically (DRY). The reporting side paginates over its
 * own tables and maps to this record.</p>
 *
