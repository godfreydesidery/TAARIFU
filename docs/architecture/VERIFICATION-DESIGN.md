...
- **Provisioning:** `setup` returns the `otpauth://totp/Taarifu:{phone}?secret=…&issuer=Taarifu` URI **once** (and the raw secret once, for manual entry); `activate {totp}` verifies a code against `mfa_pending_secret`, promotes it to `mfa_totp_secret`, sets `mfa_enabled=true`, audits `AUTH_MFA_ENROLLED`.
