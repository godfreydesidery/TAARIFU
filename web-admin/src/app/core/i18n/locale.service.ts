...
/** The locales the admin console ships with (Swahili-first per PRD §14, CLAUDE.md §5). */
 ...
 *
 * <p>Responsibility: single source of truth for the current language. Swahili is the default; English is
 * the fallback (the SW → EN → key chain is configured on {@link TranslateService}). The chosen locale is
 * persisted so it survives reloads, and exposed as a signal so the header's language switch updates
 * reactively. The {@link AuthInterceptor} reads {@link current} to set `Accept-Language` so the server
 * returns localised envelope messages (ADR-0010).</p>
 */
@Injectable({ providedIn: 'root' })
export class LocaleService {
  private static readonly STORAGE_KEY = 'taarifu.admin.locale';
  private readonly translate = inject(TranslateService);

  /** Reactive active locale; drives the header switcher and `lang` attribute. */
   ...
   * Called once from the app bootstrap (APP_INITIALIZER-style provider).
   */
  init(): void {
