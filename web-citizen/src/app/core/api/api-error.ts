 ...
 *
 * <p>Responsibility: a single, typed error shape for the whole app so components/services never parse raw
 * `HttpErrorResponse` bodies. The {@link ApiResponseInterceptor} translates every failure — envelope
 * errors, transport errors (offline/timeout), and non-envelope responses — into one of these. UI code
 * branches on the stable {@link code} (never the localised {@link message}).</p>
 */
export class ApiError extends Error {
  /**
   * @param code stable machine error code from `data.code` (e.g. `VALIDATION_FAILED`, `NOT_FOUND`,
   *   `UNAUTHENTICATED`, `TIER_TOO_LOW`), or a synthetic client code (`NETWORK_ERROR`, `UNKNOWN`) for
   *   transport failures.
  ...
  /** True when the citizen's trust tier is insufficient for the action (drives the "unlock" prompt). */
  get isTierTooLow(): boolean {
    return this.code === 'TIER_TOO_LOW';
  }

  /** True when the request reached no server (offline, DNS, CORS, timeout) — drives offline UX. */
