 ...
 *
 * <p>Responsibility: read the (non-secret) claims of an access token for UI hints ONLY — never to make a
 * security decision (the server is the real gate). Decoding only base64url-parses the payload; it does
 * NOT verify the signature (the client can't, and doesn't need to — it trusts server enforcement).
 * Hand-written rather than pulling a JWT library to keep the bundle small for low-data/low-end clients
 * (PRD §15).</p>
 */
 ...
 *
 * <p>Reads common claim shapes defensively (roles as `roles`/`authorities`/space-delimited `scope`; tier
 * as `trustTier`/`tier`). Returns `null` for an unparseable token so callers treat the user as logged
 * out.</p>
 *
...
/** Strips a leading `ROLE_` so UI checks compare on bare role names (e.g. `CITIZEN`). */
