 ...
 * surfaces a localised toast (PRD §17).
 *
 ...
 * {@link ApiError.code}). It raises a toast with the envelope's already-localised `message`, except for
 * `401` (handled by the auth interceptor) and validation errors (the form shows those inline) — avoiding
 * duplicate messaging.</p>
 *
 * <p>Security: only the server-provided localised `message` is shown — never a stack trace or raw body.
 * A transport failure (offline/CORS/timeout, `status === 0`) becomes a `NETWORK_ERROR` {@link ApiError}
 * with a generic, Swahili-first, low-data-friendly message (important on a slow Dar es Salaam link).</p>
 */
export const apiResponseInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  return next(req).pipe(
    catchError((error: unknown) => {
      ...
      // Don't toast for: 401 (auth interceptor handles it) or validation (shown inline on the form).
...
function toApiError(error: unknown): ApiError {
  ...
  if (error.status === 0) {
    return new ApiError('NETWORK_ERROR', 'Mtandao haupatikani. Angalia muunganisho wako.', 0);
  }
